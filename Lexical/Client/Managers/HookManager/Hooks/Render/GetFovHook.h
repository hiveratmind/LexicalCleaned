...
private:
	...
	static float GetFovCallback(LevelRendererPlayer* _this, float a, bool applyEffects) {
		if (!_this) return oFunc ? oFunc(_this, a, applyEffects) : 0.f;
		...
		if (_ReturnAddress() == getFovForCameraReceive) {
			auto customFovMod = ModuleManager::getModule<CustomFov>();
			if (customFovMod && customFovMod->isEnabled()) {
				return customFovMod->fov;
			}
		}

		return oFunc ? oFunc(_this, a, applyEffects) : 0.f;
	}
